...
print("[carfly_tp v4.3] loaded")

    -- Minimal & strikt:
    -- - Nur Model:PivotTo (keine Forces, keine Gravity-Tricks)
    -- - Heartbeat-Loop -> serverseitig sichtbar
    -- - W/S = ±LookVector (inkl. Pitch), KEIN Strafe
    -- - Idle: harter Lock (kein Absinken)
    -- - Garantierte Aufwärtsbewegung pro Substep (ohne Lift-Off)
    -- - Optional: SafeFly (aus standardmäßig)
    ...
    -- ===== Tuning (nur das Nötige) =====
    local DEFAULT_SPEED    = 240          -- Grundgeschwindigkeit
    local MAX_STEP_DIST    = 4            -- max TP-Distanz pro Substep
    local MAX_SUBSTEPS     = 14           -- max Substeps je Heartbeat
    local PITCH_DEADZONE   = 0.015        -- LookVector.Y-Schwelle gegen Jitter
    local MIN_ASCENT_RATE  = 36           -- min. Aufwärtsrate (Studs/s), wenn Blick nach oben
    ...
    -- ===== Core (Heartbeat + Substeps, kein Lerp im Idle) =====
    local function step(dt)
        ...
        -- IDLE: Pose hart halten, Orientierung folgt Kamera
        ...
        -- Input: nur ±LookVector (inkl. Pitch), kein seitlicher Drift
        local s = dirScalar()
        if s == 0 then
            local keep   = fly.hoverCF or curCF
            local lockCF = CFrame.new(keep.Position, keep.Position + Camera.CFrame.LookVector)
        ...
        for i = 1, substeps do
            ...
            -- Garantierter Up-TP: wenn Blick wirklich nach oben (über Deadzone) und geplantes ΔY zu klein
            local dY = target.Y - curPos.Y
            if look.Y > PITCH_DEADZONE and dY <= 0 then
                target = Vector3.new(target.X, curPos.Y + minAscPerStep, target.Z)
            elseif look.Y > PITCH_DEADZONE and dY > 0 and dY < minAscPerStep then
                target = Vector3.new(target.X, curPos.Y + minAscPerStep, target.Z)
            end
            -- (Kein Down-Clamp, kein Clearance → freie Durchdringung / Wände ok)
        ...
        fly.safeTask = task.spawn(function()
            while fly.enabled do
                if not fly.safeOn then task.wait(0.25)
                    ...
                    -- Boden locken (einfach auf Y der Oberfläche)
                    local cf = v:GetPivot()
                    local hit = workspace:Raycast(cf.Position, Vector3.new(0, -1500, 0),
                        (function()
                    ...
                    if hit then
                        local base = Vector3.new(cf.Position.X, hit.Position.Y + 2, cf.Position.Z)
                        ...
                        if SAFE_BACK and fly.enabled then
                            v:PivotTo(before); fly.hoverCF = before; fly.lastAirCF = before
                        end
                    end
                end
            end
    ...
    -- ===== UI =====
    local sec = tab:AddSection({ Name = "Car Fly v4.3 (TP-only)" })
